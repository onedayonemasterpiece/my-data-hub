...
def test_rotation_accepts_prepared_slug_and_commits_state_before_revoke() -> None:
    ...
    manager = next(
        node for node in tree.body if isinstance(node, ast.ClassDef) and node.name == "ShowcaseManager"
    )
    rotate = next(
        node
        for node in manager.body
        if isinstance(node, ast.FunctionDef) and node.name == "rotate_link"
    )
        ...
        for index, statement in enumerate(statements):
            ...
            for nested in nested_lists:
                if inspect_statement_lists(nested):
                    return True
        return (
            revoke_index is not None
            and persist_index is not None
            and persist_index < revoke_index
        )
