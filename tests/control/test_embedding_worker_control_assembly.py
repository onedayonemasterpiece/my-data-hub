...
def _persist_exact_asset_effect(ledger: ControlLedger, provider_ref: str) -> None:
    operation_id = uuid4()
    task_id = uuid4()
    ...
    intent = ProviderEffectIntent.create(
        ...
        arguments={"content_tree_sha256": "d" * 64, "control_class": "orchestrator_protected", "disposable": False},
        requested_at=now,
    )
...
def test_cold_start_lazily_assembles_same_adapter_after_exact_asset_effect(tmp_path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    adapter = object()
    settings = SimpleNamespace(assets=SimpleNamespace(dataset_ref="owner/master-assets"))
    monkeypatch.setattr(
        app_module,
    ...
    _persist_exact_asset_effect(ledger, "owner/master-assets")
    with TestClient(app):
