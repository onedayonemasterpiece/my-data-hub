...
def test_historical_notebook_source_remains_a_contract_fixture(tmp_path: Path) -> None:
...
@pytest.mark.parametrize("runner", [run_dataset_canary, run_notebook_canary])
def test_historical_canaries_are_superseded_before_ledger_or_provider(
    runner: object, tmp_path: Path
) -> None:
    receipt = tmp_path / "blocker.json"
    assert callable(runner)
    assert runner(ledger_path=tmp_path / "ledger.sqlite3", receipt_path=receipt) == EXTERNAL_BLOCKED
    payload = json.loads(receipt.read_text())
    assert payload["blocker_code"] == "SUPERSEDED_BY_CENTRAL_OPERATIONAL_MATRIX"
    assert payload["mutations_started"] == 0
    assert not (tmp_path / "ledger.sqlite3").exists()


def test_legacy_credential_helper_remains_non_mutating(
...
def test_historical_matrix_cli_is_centrally_superseded_before_adapter_or_ledger(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("KAGGLE_API_TOKEN", "must-not-create-a-second-client")
