...
def test_single_lease_restart_recovery_and_ttl(tmp_path, create_request, complete_request, terminology):
    now = [1_000_000.0]
    ...
    now[0] += 7 * 24 * 3600 + 1
    assert store.reap_expired(7 * 24 * 3600) == 1
    assert not store.session_directory(SESSION_ID).exists()
