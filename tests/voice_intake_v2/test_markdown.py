...
def test_v2_markdown_contains_contract_metadata_and_text_but_no_audio(
    ...
    assert frontmatter["transcription_request_uid"] is None
    assert frontmatter["transcription_mode"] == "bounded_per_source_chunk"
    assert frontmatter["transcription_segment_count"] == 1
    assert frontmatter["content_verification_status"] == "passed"
    assert frontmatter["content_verification_receipt_sha256"] == "d" * 64
    assert frontmatter["transcription_segments"] == [{
        "chunk_index": 0,
        "source_sha256": SHA,
        "audio_start_ms": 0,
        "audio_end_ms": 240000,
    ...
    assert "per_source_chunk_transcription" in rendered.registry_entry["quality_flags"]
    assert "content_coverage_verified" in rendered.registry_entry["quality_flags"]
    assert "aggregate_transcription_single_request" not in rendered.registry_entry["quality_flags"]
...
def test_priority_twenty_minute_projection_renders_without_legacy_chunk_limit(
    ...
    chunks = [dict(complete["chunks"][0])]
    chunks[0].update(
        {"duration_ms": 1_200_000, "audio_end_ms": 1_200_000, "wall_end_ms": 1_200_000}
    )
    complete.update(
        {"wall_elapsed_ms": 1_200_000, "recorded_audio_ms": 1_200_000, "chunks": chunks}
    )
    transport = ({"chunk_index": 0, "sha256": SHA, "duration_ms": 1_200_000},)
    rendered = render_publication(replace(item, complete=complete, transport_chunks=transport))
...
@pytest.mark.parametrize(
    ("receipt", "descriptor"),
    [
        ("", {"mode": "per_chunk", "segment_count": 1}),
        ("d" * 64, {"mode": "aggregate", "segment_count": 1}),
        ("d" * 64, {"mode": "per_chunk", "segment_count": 0}),
    ],
)
def test_full_transcript_section_fails_closed_without_verified_segment_projection(
    create_request, complete_request, terminology, receipt, descriptor
) -> None:
    item = replace(
        projection(create_request, complete_request, terminology),
        content_verification_receipt_sha256=receipt,
        transcription_limiter=descriptor,
    )
    with pytest.raises(VoiceIntakeError) as caught:
        render_publication(item)
