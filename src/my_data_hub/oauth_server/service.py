...
class AuthorizationService:
    ...
    async def validate_authorization_request(
        ...
        if "audience" in parameters and parameters["audience"] != self.settings.audience_for_resource(resource):
            raise OAuthProtocolError("invalid_target")
        try:
    ...
    def _access_response(
        self, *, subject: str, client_id: str, scopes: tuple[str, ...], now: int, resource: str
    ) -> dict[str, object]:
