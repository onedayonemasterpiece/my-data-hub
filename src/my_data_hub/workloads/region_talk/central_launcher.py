...
def render_region_talk_stage_worker_source(
    ...
    *,
    ...
    lines = [
        ...
        "        return json.loads(response.read(262144) or b'{}')",
        "def post_with_exact_replay(path,payload):",
        "    for attempt in range(12):",
        "        try: return post(path,payload)",
        "        except Exception:",
        "            if attempt==11: raise",
        ...
        "functions=tunnel=connection=None",
        "def rotate(current_binding_sha256,*,force):",
        ...
        "    if not force and expiry>now+__import__('datetime').timedelta(seconds=90): return current_binding_sha256",
        "    checkpoint_body={'schema_version':'region-talk-stage-worker-rotation-checkpoint.v1','supervisor_task_run_id':EXPECTED['supervisor_task_run_id'],'export_batch_id':EXPECTED['export_batch_id'],'worker_task_run_id':EXPECTED['worker_task_run_id'],'dispatch_id':EXPECTED['dispatch_id'],'effect_id':EXPECTED['effect_id'],'work_item_id':EXPECTED['work_item_id'],'prior_worker_generation':access['generation'],'prior_worker_binding_sha256':current_binding_sha256,'requested_at':now.isoformat(),'publication_dispatch':False,'notification_dispatch':False}",
        ...
        "    return status['worker_binding_sha256']",
        "binding_sha256=rotate(launch['worker_binding_sha256'],force=True)",
        "request_module=__import__('my_data_hub.workloads.region_talk.stage_dispatch',fromlist=['StageWorkerPayloadFetchRequest'])",
        "request=request_module.StageWorkerPayloadFetchRequest(worker_task_run_id=EXPECTED['worker_task_run_id'],dispatch_id=EXPECTED['dispatch_id'],effect_id=EXPECTED['effect_id'],worker_binding_sha256=binding_sha256,requested_at=__import__('datetime').datetime.now(__import__('datetime').UTC))",
        "def checkpoint(current_functions,current_request,*,phase):",
        "    updated_binding=rotate(current_request.worker_binding_sha256,force=False)",
        "    return functions,current_request.model_copy(update={'worker_binding_sha256':updated_binding,'requested_at':__import__('datetime').datetime.now(__import__('datetime').UTC)})",
        ...
        "terminal={'schema_version':'region-talk-stage-worker-terminal.v1','worker_task_run_id':EXPECTED['worker_task_run_id'],'dispatch_id':EXPECTED['dispatch_id'],'effect_id':EXPECTED['effect_id'],'master_instance_id':EXPECTED['master_instance_id'],'epoch':EXPECTED['epoch'],'result_status':result.result_status.value,'result_receipt_sha256':result_sha,'completed_at':__import__('datetime').datetime.now(__import__('datetime').UTC).isoformat(),'publication_dispatch':False,'notification_dispatch':False}",
        "post_with_exact_replay('/internal/region-talk-pipeline/stage/terminal',terminal)",
