...
class ShowcaseGatewaySettings:
    ...
    def from_env(cls) -> ShowcaseGatewaySettings:
        ...
        if not raw_token_file:
            raise ShowcaseGatewayError(
                "MY_DATA_HUB_SHOWCASE_GATEWAY_TOKEN_FILE is required"
            )
        ...
        try:
            ...
            raise ShowcaseGatewayError(
                "MY_DATA_HUB_SHOWCASE_GATEWAY_TIMEOUT_SECONDS must be numeric"
            ) from exc
...
def _validate_gateway_url(raw_url: str) -> str:
    ...
    if parts.path.rstrip("/") != SHOWCASE_GATEWAY_PATH:
        raise ShowcaseGatewayError(
            f"showcase gateway URL path must be exactly {SHOWCASE_GATEWAY_PATH}"
        )
    if parts.scheme == "http" and parts.hostname not in {
        ...
        raise ShowcaseGatewayError(
            "plain HTTP showcase gateway is allowed only on loopback"
        )
...
def _read_service_token(path: Path) -> str:
    ...
    if stat.S_IMODE(file_stat.st_mode) & 0o077:
        raise ShowcaseGatewayError(
            "showcase gateway token file must not be readable by group or others"
        )
    try:
