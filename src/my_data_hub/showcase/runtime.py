...
class ShowcaseRuntimeSettings:
    ...
    def from_env(cls) -> ShowcaseRuntimeSettings:
        ...
        if not raw_token:
            raise ShowcaseRuntimeAuthenticationError(
                "MY_DATA_HUB_SHOWCASE_RUNTIME_TOKEN_FILE is required"
            )
        artifact_root = Path(
        ...
        if host not in {"127.0.0.1", "localhost", "::1"}:
            raise ShowcaseRuntimeRequestError(
                "showcase runtime must bind to a loopback address"
            )
        try:
            port = int(os.getenv("MY_DATA_HUB_SHOWCASE_RUNTIME_PORT", "8790"))
            max_request_bytes = int(
                os.getenv("MY_DATA_HUB_SHOWCASE_MAX_REQUEST_BYTES", "65536")
            )
        except ValueError as exc:
            raise ShowcaseRuntimeRequestError(
                "showcase runtime numeric configuration is invalid"
            ) from exc
        if not 1 <= port <= 65535:
            raise ShowcaseRuntimeRequestError("showcase runtime port is invalid")
        if not 4096 <= max_request_bytes <= 262_144:
            raise ShowcaseRuntimeRequestError(
                "showcase runtime request limit must be 4 KiB..256 KiB"
            )
        ...
        settings = cls(
            ...
            site_template_dir=(
                Path(template_raw).expanduser().resolve() if template_raw else None
            ),
            site_runtime_dir=(
                Path(runtime_raw).expanduser().resolve() if runtime_raw else None
            ),
        )
...
def read_runtime_token(path: Path) -> str:
    try:
        ...
        raise ShowcaseRuntimeAuthenticationError(
            "showcase runtime token file is unavailable"
        ) from exc
    if not stat.S_ISREG(file_stat.st_mode):
        raise ShowcaseRuntimeAuthenticationError(
            "showcase runtime token path must be a regular file"
        )
    if stat.S_IMODE(file_stat.st_mode) & 0o077:
        raise ShowcaseRuntimeAuthenticationError(
            "showcase runtime token file must not be readable by group or others"
        )
...
def _sanitize_list_result(value: Any) -> Any:
    ...
    for raw_key, child in value.items():
        key = str(raw_key)
        lowered = key.lower()
        if isinstance(child, str) and child.startswith(("http://", "https://")) and (
            "url" in lowered or "link" in lowered
        ):
...
class ShowcaseOperationController:
    ...
    def invoke(
        ...
        required_scope = "showcase:write" if tool in SHOWCASE_WRITE_TOOLS else "showcase:read"
        ...
        with self._lock:
            ...
            if isinstance(completed, Mapping) and isinstance(
                completed.get("result"), (dict, list)
            ):
    ...
    def _rotate(
        ...
        *,
        ...
        if pending and pending.get("idempotency_key") != idempotency_key:
            raise ShowcaseRuntimeConflictError(
                "another rotation for this view is awaiting recovery"
            )
