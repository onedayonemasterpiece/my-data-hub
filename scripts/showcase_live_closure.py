...
async def run() -> int:
    ...
    token = os.environ.get("MY_DATA_HUB_MCP_ACCEPTANCE_OPERATOR_TOKEN", "").strip()
    ...
    try:
        ...
        require(24 <= len(token) <= 4096 and not any(char.isspace() for char in token), "MCP_TOKEN_MISSING")
        async with (
            httpx2.AsyncClient(
            ...
            async with ClientSession(read_stream, write_stream, read_timeout_seconds=90) as session:
                ...
                require(changed_source["view"].get("access_label", "").endswith(marker), "MAIN_SOURCE_CHANGE_NOT_VISIBLE")
                ...
                require(rolled_back.get("status") in {"published", "applied_not_published"}, "MAIN_ROLLBACK_APPLY_FAILED")
                ...
                eligible_items = [
                    item for item in original_source["items"] if item.get("capability_type") in {"technical", "product", "business"}
                ]
        ...
        receipt["cleanup"] = {
            "main_rollback_completed": receipt.get("checks", {}).get("main_content_cycle", {}).get("rollback_exact_bundle") is True,
