...
| R04 chunked private Dataset create | Done | Added `provider.upload.start`, `provider.upload.put_chunk`, `provider.upload.status`, `provider.upload.finalize`, and `provider.upload.abort`; all five route through the production `LedgerControlReader` to the single injected provider gateway, and finalization uses its existing durable intent/claim/idempotency path. Existing create/version and download tools remain compatible. |
| Exact identity and replay binding | Done | Upload ID, task, resource, OAuth subject/client, effect, idempotency request, manifest, offsets and hashes are exact-bound. Start/chunk/finalize/abort response-loss replays are deterministic; changed replays quarantine or conflict. `FINALIZING` is restart-reconcilable through the same provider intent. A valid terminal receipt is authoritative over an orphan active directory after any crash window. If terminalization wins between the receipt precheck and active-lock acquisition, start/status/finalize/abort re-read and return the exact bound receipt instead of a transient not-found; put-chunk revalidates the receipt and returns the defined terminal conflict. Concurrent receipt readers remove an orphan idempotently without accepting observed unsafe filesystem objects. |
...
- deterministic start/status/finalize/abort terminalization races between receipt precheck and active lock;
- deterministic put-chunk vs finalize/abort races and two-reader orphan cleanup;
- exact production `LedgerControlReader` dispatch coverage for all five upload tools;
...
- `python scripts/validate_repository.py`: PASS (`4445` checks, zero errors)
