...
- FM14 deterministic task-owned corruption/readback rejection and FM15 fixed
  failing isolated verifier rejection, both without HEAD mutation.
...
No new wire schema was added: the adapter consumes and emits the already
validated checkpoint-acceptance intent/stage/receipt contracts. The runtime
binding is owner-local provider configuration and is deliberately not a remote
caller payload.
...
- `python scripts/validate_repository.py`: PASS, 3,389 checks, zero errors.
- `pytest`: PASS, 838 passed / 3 skipped.
- `ruff check .`: PASS.
- `mypy`: PASS.
