...
| FM09 | `ControlLedgerStoredReplay` selects one canonical ACKed event from the protected ledger, replays it by ID with its exact derived token, retries the same body with a genuinely revoked prior-run token, derives a fixed older-epoch envelope, and hashes operation/service/event state before/after. Callback bytes never cross the request or operator boundary. |
