...
| FM09 | `ControlLedgerStoredReplay` selects one canonical ACKed event from the protected ledger and hashes operation/service/event state before/after. Exact current and genuinely revoked prior-run bearer values must come from a runtime-owned protected replay capability; the control plane stores only hashes and blocks when that capability is absent. The fixed older-epoch envelope is still derived from non-secret identity. Callback bytes and bearer values never cross the request or operator boundary. |
